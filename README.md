...
* the next message sends all of the unicode control characters are specifically tested, including 0, 127 and all of the characters from 128 to 159 (x80 - x9F). The null (0) that is sent is what Rust automatically sends to represent 0. Until I can read the violations.log with a binary editor, I can't guarantee that it was a true zero and not some kind of an escaped zero.

* the final message sends a string of unstructured text. It doesn't pretend to be anything that resembles a JSON message. This verifies that non-JSON messages will be recorded without panics.

Both the csp endpoint and reqs log results thru logging macros. Most log entries are at info level but failures are set to the warn level. The logging messages are displayed as plain text in the terminal/command window. If desired, set the default logging level to debug to display some more information. The trace level may display some messages that are set by the third-party libraries.
